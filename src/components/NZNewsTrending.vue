<template>
  <v-container id="nz-news-trending" fluid class="pa-0">
    <v-card elevation="0" tile fill-height>
      ...
      <!-- <div v-for="(news, index) in newsList" :key="index">
        <v-divider></v-divider>
        <v-card-subtitle class="py-2 text-h6">
        ...
        <v-col justify="start" class="px-2 pt-0 pb-2 text-body-2">
          <span class="group pl-0">
            <v-icon>icon-time</v-icon>{{ news.date }}
            <v-icon class="ml-1">icon-user</v-icon>{{ news.author }}
            <v-icon class="ml-1">icon-view</v-icon>{{ news.views }}
            <v-icon class="ml-1">icon-comment</v-icon>{{ news.comments }}
          </span>
        </v-col> -->
      <div v-for="(trendingInfo, index) in trendingList" :key="index">
        <v-divider></v-divider>
        <v-card-subtitle class="text-over2-hidden py-2">
          <a :href="transHtml(trendingInfo.meta.html_url)"> {{ trendingInfo.title }}</a>
        </v-card-subtitle>
        <v-col justify="start" class="px-2 pt-0 pb-2 text-body-2">
          <span class="group pl-0">
        ...
        <!-- <v-toolbar height="30" elevation="0" class="text-caption justify py-0">
          <v-row no-gutters>
            ...
            <v-col class="text-truncate py-0" cols="2">
              <v-icon class="icon-view py-0"></v-icon>{{ computeViews(trendingInfo.views) }}
            </v-col>
            <v-spacer></v-spacer>
            <v-col class="text-truncate py-0" cols="2">
...
export default {
  ...
  methods: {
    ...
    transHtml(html_url) {
      return html_url.replace("news", "#/news");
    },
  },
  computed: {},
};
